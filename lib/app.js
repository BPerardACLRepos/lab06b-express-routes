...
app.get('/board_games', async (req, res) => {
  try {
    const data = await client.query(
      `SELECT 
      board_games.name, board_games.max_players, board_games.min_players, board_games.expansion, board_games.category_id, board_games.id, board_games.owner_id, categories.category 
      FROM 
      board_games
      JOIN
      categories
      ON
      board_games.category_id = categories.id`);
